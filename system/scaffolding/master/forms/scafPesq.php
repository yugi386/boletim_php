...
// CODIGO PARA EVITAR EXECUCAO DE BLOCOS SEM PASSAR PELA PAGINA INICIAL
	...
	$cab = array_keys($FW_campos);
	$campos = array_values($FW_campos);
	$limit = count($FW_campos);
...
<form method="post" action="<?php echo FW_RAIZ . $view_redir ;?>index" id="frm-filtro">
	<p>
		...
		<input type="text" id="fw_pesquisar" name="fw_pesquisar" size="30" value="<?php echo $pesq; ?>" />		
		&nbsp;&nbsp;&nbsp;
		<select name="fw_relacional" id="fw_relacional">
			...
			<?php if ($relac == "3") { ?>
				<option value="3" selected>IGUAL À
			<?php } else { ?>	
				<option value="3">IGUAL À
			<?php } ?>		
